(function () {
  ...
  function initHeroParallax() {
    ...
    // Mobile / touch: CSS ambient drift only (no JS loop = no lag)
    if (isTouch || window.innerWidth < 900) {
      img.classList.add("is-drifting");
      return;
    }
    ...
    function tick() {
      ...
      cx += (mx - cx) * 0.05;
      cy += (my - cy) * 0.05;
      // combine subtle mouse parallax on top of CSS drift container
      bg.style.transform = "translate3d(" + (cx * -10) + "px," + (cy * -6) + "px,0)";
